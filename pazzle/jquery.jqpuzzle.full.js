(function($) {
    ...
    $.fn.jqPuzzle = function(settings, texts) {
        ...
        var defaults = {
            ...
            autoSave:true,                        //是否自动保存进度[true|false]
            // display additional gui controls
            control: {
                saveProcess: false, //display 'save' button [true|false] 
                preStatus: false,  //display 'pre status' button [true|false]
        ...
        moveAct = settings.moveAct,
        getRewardImg = settings.getRewardImg;
        ...
        return this.filter('img').each(function(){
            ...
            $srcImg.one('load', function() {
                ...
                var  bgSize = eval("'"+fullWidth+"px "+fullHeight+"px'");
                ...
                .css({
                    ...
                    visibility: 'inherit',
                    cursor: 'default'
                }).css("-webkit-background-size",bgSize) //Justin add here
                ...
                var rewardImg ;
                if($.isFunction(getRewardImg.callback)){
                    rewardImg = getRewardImg.callback();
                }
                if(!rewardImg){
                    rewardImg=imgSrc;
                }
                ...
                var $shuffleButton, $originalButton, $numbersButton,$preStatusButton;
			
                // create a button prototype to be cloned for the actual buttons
                var $protoButton = $('<a/>').css('cursor', 'default');
                        
                //caeate preStatus button
                if(control.preStatus){
                    $preStatusButton = $protoButton.clone()
                    .text(texts.preStatusLabel)
                    .appendTo($controls);
                }
                        
                //create save button
                if(control.saveProcess){
                    $saveButton = $protoButton.clone()
                    .text(texts.saveStatusLabel)
                    .appendTo($controls);
                }
                ...
                $pieces.click(function() {
                    ...
                    //移动动作
                    if($.isFunction(moveAct.callback)){
                        moveAct.callback("");
                    }
                ...
                //shuffle pieces to pre status, Justin add here
                if(control.preStatus) $preStatusButton.click(function() {
                    //shuffle(1, animation.shuffleSpeed);
                    shuffle(1);
                });
                        
                //save button on click ,Justin add here 
                if(control.saveProcess) $saveButton.click(function() {
