...
AC.VR.options = {
	...
	noCache: true,             // (boolean)  append a random query string to the image URLs?
	initialPos: [-1,0],          // (mixed)    initial VR position (number or array)
...
Object.extend(AC.VR.prototype, {
	...
	initialize: function(container, imagePath, totalFrames, options){
		...
		// Notice Icon		
		this.icon = $(document.createElement('div')).addClassName('icon');
 		this.container.appendChild(this.icon);
        this.icon.setAttribute('id',"icon");
		if(this.options.invert[0]){
		    this.icon.addClassName("arrow_leftright");
			this.icon.removeClassName("arrow_updown");
		}else{ 
		    this.icon.addClassName("arrow_updown");
		    this.icon.removeClassName("arrow_leftright");
		}
		//alert(this.icon);
		//alert(this.icon.getAttribute("id"));
		//var icondiv = this.icon;		
		/*this.icon.addEventListener("touchstart", function(event){		   
			icondiv.style.display="none";
		},false);
		this.icon.addEventListener("touchend", function(event){		   
			icondiv.style.display="";
		},false);
		//this.icon.addEventListener("touchend", this.bindGrabChange,false);
        */
	...
	unmakeInteractive: function(){
		...
		if (this.mobile) {
			...
			//this.icon.down().stopObserving('touchmove', this.bindGrabChange);
			//this.icon.down().stopObserving('touchend', this.bindGrabEnd);
		}
		
		if (this.options.grabbable) {
			// grab events
			this.vr.stopObserving(this.getStr('mousedown'), this.bindGrabStart);
			//this.icon.stopObserving(this.getStr('mousedown'), this.bindGrabStart);
			...
			//this.icon.stopObserving('click', this.bindOnClick);
			//this.icon.stopObserving('focus', this.bindOnFocus);
			//this.icon.stopObserving('blur', this.bindOnBlur);
		}
	...
	onGrabStart: function(event){
		...
		if (this.mobile) {
			...
			//this.icon.down().observe('touchmove', this.bindGrabChange);
			//this.icon.down().observe('touchend', this.bindGrabEnd);
		}
