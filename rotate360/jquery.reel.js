        ...
        // Events & handlers
          ...
          */
            ...
            var
              ...
              area_limit = {
            		  left: number(t[0].offsetLeft ? t[0].offsetLeft : t.parent().parent()[0].offsetLeft), 
            		  top: number(t[0].offsetTop ? t[0].offsetTop :t.parent().parent()[0].offsetTop),
            		  right: number(t[0].offsetLeft ? t[0].offsetLeft : t.parent().parent()[0].offsetLeft) + size.x,
                      bottom: number(t[0].offsetTop ? t[0].offsetTop :t.parent().parent()[0].offsetTop) + size.y
              };
            set("area_limit",area_limit);
        ...
        on= {
          ...
          */
            var
              ...
              $hi= $(_div_tag_, { 'class': hi_klass,
                css: { position: _absolute_, left: 0, top: 0, width: space.x, height: space.y, background: _hex_black_, opacity: 0 } ,  id: "interface_" + id,
              }).appendTo($overlay),
			...
			$overlay= t.parent();
			var cls = opt.horizontal ? "icon arrow_leftright" : "icon arrow_updown";
			$overlay.append($icon = $(_div_tag_, { 'class': cls,
              css: {
			    overflow:_hidden_,
              },
			  id: "icon_"+ get(_id_),
            }));
			$icon.bind(_touchstart_, function(e){ t.trigger('down', [finger(e).clientX, finger(e).clientY, true]); })
			     .bind(opt.clickfree ? _mouseenter_ : _mousedown_, function(e){ t.trigger('down', [e.clientX, e.clientY]); return false })
            icon = $icon;
			t.trigger('preload');
          },
          ...
          */
            if (opt.draggable && slidable){
              ...
              unidle();
              var
                limit = get("area_limit"),
                inLimit = (x > limit.left && x < limit.right) && ( y > limit.top && y< limit.bottom),  
                delta= { x: x - last.x, y: y - last.y }
              if (inLimit && (abs(delta.x) > 0 || abs(delta.y)) > 0){
                last= { x: x, y: y };
                var
